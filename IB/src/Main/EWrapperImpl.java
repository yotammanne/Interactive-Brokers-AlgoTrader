...
import TickTypes.DelayedLastPrice;
import TickTypes.RealTimeBar;
...
public class EWrapperImpl implements EWrapper {
	...
	public static Map<Integer,Queue<Tick>> dataTicks = new HashMap<>();
	//! [socket_declare]
	...
	public void updateDataTicks(int id, Tick tick) {
		if(!dataTicks.containsKey(id)) {
			dataTicks.put(id, new LinkedBlockingQueue<Tick>());
			dataTicks.get(id).add(tick);
		}
		else {
			dataTicks.get(id).add(tick);
		}
	...
	public void tickPrice(int tickerId, int field, double price, TickAttr attribs) {
		...
		if(field==67) {
			Tick tick = new DelayedLastPrice(price);
			this.updateDataTicks(tickerId, tick);
			Utils.handleTick(tickerId,tick);
		}
	}
	//! [tickprice]
	...
	public void realtimeBar(int reqId, long time, double open, double high,
		...
		Tick tick= new RealTimeBar(time,open,high,low,close,volume,wap,count);
		this.updateDataTicks(reqId,tick);
		Utils.handleTick(reqId,tick);
